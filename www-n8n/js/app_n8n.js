document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const resp = await N8N_API.getRestaurants();
  ...
  async function loadTasks(selectedRestaurant = "") {
    try {
      ...
      const res = await N8N_API.getTasks(selectedRestaurant);
  ...
  document.getElementById("save-profile-btn").addEventListener("click", async () => {
    ...
    if (first && last) {
      try {
        const resp = await N8N_API.updateProfile({ first_name: first, last_name: last });
        if (resp && resp.success) {
          localStorage.setItem("user_profile_data", JSON.stringify(resp.data?.user));
          document.getElementById("user-fullname").textContent = resp.data?.user?.fullName;
        } else {
